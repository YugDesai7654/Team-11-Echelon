...
def main():
    ...
    with col2:
        ...
        st.markdown("#### Options")
        use_search = st.checkbox("Enable Internet Search context (simulated via Gemini Knowledge)", value=True)
    ...
    if analyze_btn:
        if claim_text:
            ...
            with st.spinner("consulting Gemini knowledge base and analyzing media consistency..."):
                try:
                    # Import here to avoid issues if not running
                    from src.analysis import detect_misinformation
                    
                    # Mock CLIP score (to be replaced by teammate's module)
                    # We pass this to help Gemini understand the visual-text link
                    mock_clip_score = 0.28 
                    
                    # Perform Analysis
                    result = detect_misinformation(
                        claim_text, 
                        media_content=image if image else None, 
                        clip_score=mock_clip_score
                    )
            ...
            if result:
                ...
                with st.expander("View Raw Analysis Data"):
                    st.json(result)
                    st.metric("Internal CLIP Consistency", f"{mock_clip_score:.2f}")
