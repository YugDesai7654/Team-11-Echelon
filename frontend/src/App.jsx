...
import {
  ...
  Bot, ShieldAlert, Activity, FileText
} from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white selection:text-black">
      {/* Header */}
      <header className="border-b border-zinc-900/50 px-6 py-4 flex items-center justify-between sticky top-0 bg-black/80 backdrop-blur-xl z-50">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 bg-white rounded-sm flex items-center justify-center">
            <span className="text-black font-bold text-xs">E</span>
          </div>
          <span className="font-bold tracking-tight text-lg">ECHELON</span>
        </div>
        <div className="text-zinc-500 text-sm">Team 11</div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 space-y-16">
        ...
        <section className="max-w-4xl mx-auto">
          <div className="text-center space-y-4 mb-8">
            ...
            <p className="text-zinc-400 text-lg">
              Advanced multi-modal misinformation detection pipeline.
            </p>
          </div>

          <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 backdrop-blur-sm shadow-2xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* File Upload */}
              <div className="space-y-3">
                <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                  <Upload className="w-3 h-3" /> Evidence Media
                </label>
                <div className="relative group h-48">
                  ...
                  />
                  <div className={cn(
                    "h-full border-2 border-dashed border-zinc-800 rounded-xl flex flex-col items-center justify-center transition-all duration-300",
                    "group-hover:border-zinc-600 group-hover:bg-zinc-900",
                    file ? "border-white/20 bg-white/5" : ""
                  )}>
                    {file ? (
                      <div className="text-center p-4">
                        {file.type.startsWith('video') ? (
                          <FileVideo className="w-10 h-10 text-white mx-auto mb-3" />
                        ) : (
                          <FileImage className="w-10 h-10 text-white mx-auto mb-3" />
                        )}
                        <span className="block text-white font-medium truncate max-w-[200px]">{file.name}</span>
                        <span className="text-xs text-zinc-500 mt-1">{(file.size / 1024 / 1024).toFixed(2)} MB</span>
                        <div className="mt-2 text-xs text-green-400 font-mono flex items-center justify-center gap-1">
                          <CheckCircle2 className="w-3 h-3" /> Ready
                        </div>
                      </div>
                    ) : (
                      <div className="text-center p-4 space-y-2">
                        <div className="w-10 h-10 bg-zinc-800 rounded-full flex items-center justify-center mx-auto mb-2 text-zinc-400">
                          <Upload className="w-5 h-5" />
                        </div>
                        <span className="text-zinc-300 font-medium">Upload Evidence</span>
                        <p className="text-xs text-zinc-500">JPG, PNG, MP4 supported</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-3 flex flex-col">
                <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                  <FileText className="w-3 h-3" /> Claim / Caption
                </label>
                <textarea
                  className="flex-1 bg-zinc-950/50 border border-zinc-800 rounded-xl p-4 text-white placeholder:text-zinc-700 focus:outline-none focus:ring-1 focus:ring-white/20 transition-all resize-none font-sans text-sm leading-relaxed"
                  placeholder="Paste the caption or claim you found with this media..."
                  value={text}
            ...
            <div className="mt-6 pt-6 border-t border-zinc-800 flex justify-end">
              <button
                ...
                className={cn(
                  "px-8 py-3 rounded-lg font-bold text-sm tracking-wide transition-all duration-300",
                  loading
        ...
        {result && !loading && (
          <div ref={resultsRef} className="space-y-12 animate-in fade-in slide-in-from-bottom-10 duration-700 ease-out">
            ...
            {/* Inline Chat for "Single Page" feel */}
            <ChatSection
...
function LoadingView() {
  return (
    <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-2xl flex flex-col items-center justify-center space-y-6">
      <div className="w-full max-w-xs space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-center text-xs text-zinc-500 font-mono animate-pulse">Running advanced algorithms...</p>
        </div>
      </div>
    </div>
  );
...
function ResultsView({ result }) {
  ...
  if (score >= 80) {
    ...
    bgGradient = "from-green-950/20 to-zinc-950";
  } else if (score >= 50) {
    verdictColorClass = "text-orange-500";
  ...
  return (
    <div className="space-y-8">
      {/* Verdict Header */}
      <div className={cn("relative rounded-3xl p-8 md:p-12 overflow-hidden border bg-gradient-to-br transition-colors", borderColorClass, bgGradient)}>
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            <div className="inline-flex items-center gap-2 mb-6 px-3 py-1 rounded-full bg-black/40 border border-white/5 backdrop-blur-sm">
            ...
            <h2 className={cn("text-6xl md:text-7xl font-black tracking-tighter mb-6", verdictColorClass)}>
              {verdict.toUpperCase()}
            </h2>

            <div className="bg-black/20 border border-white/5 rounded-xl p-6 backdrop-blur-md">
          ...
          <div className="flex flex-col items-center justify-center">
            <div className="relative w-56 h-56 flex items-center justify-center group">
              <svg className="w-full h-full -rotate-90 transform transition-transform group-hover:scale-105 duration-500" viewBox="0 0 100 100">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-6xl font-bold text-white">{Math.round(score)}</span>
...
function DetailedAnalysis({ result }) {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-2xl font-bold text-white px-2">Pipeline Analysis Details</h3>

      <div className="grid grid-cols-1 gap-4">
        {/* D1: Input Handling */}
        <DetailCard title="D1: Multi-Modal Input" icon={FileImage}>
        ...
        {/* D2: Cross Modal */}
        <DetailCard title="D2: Cross-Modal Inconsistency" icon={LinkIcon}>
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <DetailItem label="Confidence" value={`${(raw.cross_modal_analysis?.confidence * 100).toFixed(1)}%`} />
            </div>
        ...
        {/* D3: Context */}
        <DetailCard title="D3: Out-of-Context Detection" icon={Search}>
          <div className="space-y-4">
            <div className="flex gap-8 items-center border-b border-zinc-800 pb-4">
              <div className="text-center">
                <div className="text-3xl font-mono font-bold">{contextData.context_score?.toFixed(2)}</div>
                <div className="text-xs uppercase text-zinc-500 tracking-wider">Context Score</div>
              </div>
              <div>
                <div className="text-lg font-bold text-white">{contextData.context_verdict}</div>
                ...
                <ul className="space-y-2">
                  {contextData.search_results.results.slice(0, 3).map((res, i) => (
                    <li key={i} className="bg-zinc-950/50 p-3 rounded border border-zinc-900 flex justify-between items-center group hover:border-zinc-700 transition-colors">
                      <a href={res.url} target="_blank" rel="noreferrer" className="text-sm text-blue-400 hover:underline truncate max-w-[80%]">
                        {res.title}
                      </a>
        ...
        <DetailCard title="D4: Synthetic Media Detection" icon={Bot}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center bg-zinc-950 p-3 rounded border border-zinc-900">
                <span className="text-sm text-zinc-500">AI Probability</span>
                <span className={cn("font-mono font-bold", syntheticData.ai_text_probability > 0.5 ? "text-red-400" : "text-green-400")}>
                  {(syntheticData.ai_text_probability * 100).toFixed(1)}%
                </span>
              </div>
              <div className="flex justify-between items-center bg-zinc-950 p-3 rounded border border-zinc-900">
                <span className="text-sm text-zinc-500">Classifier Label</span>
                <span className="text-sm text-white">{syntheticData.text_label}</span>
              </div>
            </div>
              ...
              {syntheticData.video_detection ? (
                <>
                  <div className="flex justify-between items-center bg-zinc-950 p-3 rounded border border-zinc-900">
                    ...
                    <span className={cn("font-mono font-bold", syntheticData.deepfake_probability > 0.5 ? "text-red-400" : "text-green-400")}>
                      {(syntheticData.deepfake_probability * 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="flex gap-2 justify-end">
                    {syntheticData.is_deepfake && <span className="text-xs bg-red-500 text-white px-2 py-1 rounded">DEEPFAKE</span>}
                  </div>
                </>
              ) : (
                <div className="flex justify-between items-center bg-zinc-950 p-3 rounded border border-zinc-900">
                  <span className="text-sm text-zinc-500">AI Image Prob</span>
                  <span className={cn("font-mono font-bold", syntheticData.ai_image_probability > 0.5 ? "text-red-400" : "text-green-400")}>
                    {(syntheticData.ai_image_probability * 100).toFixed(1)}%
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* D6: Robustness */}
          <DetailCard title="D6: Robustness" icon={ShieldCheck}>
            <div className="flex items-center justify-between">
              <span className="text-zinc-400 text-sm">Adversarial Resistance</span>
              <span className={cn("text-xl font-bold", robustnessData.robustness_score > 0.8 ? "text-green-400" : "text-orange-400")}>
                {(robustnessData.robustness_score * 100).toFixed(1)}%
              </span>
            </div>
            <div className="mt-4 space-y-2">
              {robustnessData.adversarial_flags?.length > 0 ? (
                robustnessData.adversarial_flags.map((flag, i) => (
                  <div key={i} className="text-xs text-red-400 bg-red-950/30 p-2 rounded flex gap-2">
                    <XCircle className="w-3 h-3" /> {flag}
                  </div>
                ))
              ) : (
                <div className="text-xs text-green-400 bg-green-950/30 p-2 rounded flex gap-2 items-center">
                  <CheckCircle2 className="w-3 h-3" /> No adversarial patterns detected
                </div>
              )}
            </div>
          </DetailCard>
          ...
          {/* D7: Evaluation */}
          <DetailCard title="D7: Pipeline Metrics" icon={Activity}>
            <div className="space-y-3">
              <MetricRow label="Explanation Quality" value={evalData.metrics?.explanation_quality} />
              <MetricRow label="Robustness" value={evalData.metrics?.robustness_score} />
...
function DetailCard({ title, icon: Icon, children }) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-sm transition-all hover:border-zinc-700">
      <button
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="text-center">
          <h3 className="text-2xl font-bold flex items-center justify-center gap-2">
            <Bot className="w-6 h-6" /> Chat with Evidence
          </h3>
          <p className="text-zinc-500 text-sm mt-1">Ask questions about the analysis results</p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col h-[500px]">
          <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent" ref={scrollRef}>
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-zinc-600 space-y-2">
                ...
                <p>Ask "Why is this considered fake?" or "What evidences were found?"</p>
              </div>
            )}
            {messages.map((msg, i) => (
            ...
            {loading && (
              <div className="bg-zinc-800 text-zinc-200 mr-auto rounded-2xl rounded-tl-sm p-4 w-16">
          ...
          <form onSubmit={onSubmit} className="p-4 bg-zinc-950 border-t border-zinc-800 flex gap-2">
            <input
              className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-white/20 transition-all text-white placeholder-zinc-600"
            ...
            <button
              ...
              className="bg-white text-black p-3 rounded-xl disabled:opacity-50 hover:bg-zinc-200 transition-colors"
            >
...
const MetricCard = ({ title, value, subtitle, type = "score", invert = false, icon }) => {
  ...
  if (typeof value === 'number') {
    if (type === "percentage") {
      ...
      statusColor = isBad ? "text-red-400" : "text-green-400";
    } else {
      const isGood = invert ? value < 0.5 : value > 0.3;
      statusColor = isGood ? "text-green-400" : "text-orange-400";
    }
  }

  return (
    <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-2xl flex flex-col justify-between backdrop-blur-sm transition-all hover:bg-zinc-800/40 hover:border-zinc-700">
...
const DetailItem = ({ label, value, status }) => {
  ...
  if (status === 'good') color = "text-green-400";
  if (status === 'bad') color = "text-red-400 text-xs"; // Highlight bad status

  return (
    <div>
      <span className="text-xs text-zinc-500 uppercase tracking-wider block mb-1">{label}</span>
      <span className={cn("font-medium text-sm truncate block", color)}>{value}</span>
    </div>
  )
}

const MetricRow = ({ label, value }) => (
  <div className="flex items-center justify-between p-2 rounded hover:bg-zinc-800/30 transition-colors">
    <span className="text-sm text-zinc-400">{label}</span>
    <span className="text-sm font-bold font-mono">{(value * 100).toFixed(1)}%</span>
  </div>
)
